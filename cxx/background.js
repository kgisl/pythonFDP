...
(function(){
    ...
    function setDefaultSuggestion(text) {
        if (text) {
            chrome.omnibox.setDefaultSuggestion({"description":"<url><match>C++ Search</match></url> " + text});
        } else {
            chrome.omnibox.setDefaultSuggestion({"description":"<url><match>C++ Search</match></url>"});
        }
    };
    ...
    chrome.omnibox.onInputStarted.addListener(function(){
        ...
        if (localStorage.hasUnexpired('stl')) {
            ...
            xhr("http://tinf2.vub.ac.be/~dvermeir/mirrors/www.sgi.com/Technology/STL/table_of_contents.html",
            function(url, req) {
        ...
        if (localStorage.hasUnexpired('cplusplus_headers')) {
            cpp_ = localStorage.getObject('cplusplusheaders');
        } else {
            ...
            function(url, req) {
                ...
                var matches = text.match(new RegExp("<a href=\"/w/c/[^\"]*\".*>[^<]*<\/a>", "g"));
                for (var i = 0; i < matches.length; ++i) {
                    var match = matches[i];
                    var hrefstartidx = match.indexOf("href=\"") + 6;
                    var hrefendidx = match.indexOf("\"", hrefstartidx);
    ...
    chrome.omnibox.onInputChanged.addListener(function(text, suggest_callback) {
        ...
        if (cpp_) {
            for (var key in cpp_) {
                if (key.startsWith(qlower) && (key == cpp_[key]["name"])) {
                    var item = cpp_[key];
                    var name = item["name"];
                    var url = item["url"];
